...
import { faX, faHeart, faLocationDot, faCalendarDays, faBriefcase } from '@fortawesome/free-solid-svg-icons'
...
export default function FeedCard({ item, type, onReject, onLikeSection }: FeedCardProps) {
    ...
    return (
      <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-lg overflow-hidden">
        ...
        <div className="px-6 pt-6 mb-6 bg-white rounded-xl">
          <h2 className="text-4xl font-bold">{applicant.name}</h2>
        </div>
        ...
        <div className="px-6 mb-6">
          <div className="divide-y border-y">
            <div className="py-4 flex items-center gap-4">
              ...
              <span>{applicant.professionalExperiences?.[0]}</span>
            </div>
            <div className="py-4 flex items-center gap-4">
              <span className="text-xl text-gray-600">
        ...
        <div className="px-6 space-y-6 mb-20">
          {applicant.prompts && applicant.prompts.map((prompt, index) => (
            <div 
              key={index} 
    ...
    <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-lg overflow-hidden">
      ...
      <div className="px-6 space-y-6 mb-6">
        <div className="bg-white rounded-xl p-6 shadow-md relative">
          <p><span className="font-semibold">Team Size:</span> {company.employeeCount} employees</p>
          <p><span className="font-semibold">Operating for:</span> {company.yearsOfOperation} years</p>
          <p><span className="font-semibold">Funding:</span> {company.fundingRound}</p>
      ...
      {/* Prompts Section */}
      <div className="px-6 space-y-6 mb-20">
        {company.prompts && company.prompts.map((prompt, index) => (
          <div 
            key={index} 
            className="bg-white rounded-xl p-6 shadow-md relative"
          >
